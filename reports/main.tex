...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Relatório de Análise Estatística e Inferência Bayesiana}
...
A Figura \ref{fig:scatter_client13} explora a relação entre o RTT de Upload e a Porcentagem de Perda de Pacotes (\texttt{packet\_loss\_percent}), segmentada por servidor, reforçando achados anteriores de \texttt{EDA}. A grande maioria das medições exibe um \text{RTT} de Upload consistentemente baixo (abaixo de $0.025$ segundos), com a perda de pacotes crônica e persistente concentrada entre $2.5\%$ e $8.0\%$. Não se observa correlação linear forte entre as variáveis, sugerindo que o \text{RTT} alto de certos \textit{outliers} (próximo a $0.09$s) é causado por eventos de latência intermitentes, e não pelo mecanismo que gera a perda. A dispersão mista dos servidores na região de alta densidade indica que tanto o \text{RTT} base quanto a perda crônica são características estáveis e independentes do servidor específico.
...
A Figura \ref{fig:scatter_server02} apresenta a relação entre o RTT de Upload e a perda de pacotes especificamente para o Servidor 2. A grande maioria das observações está concentrada próxima a zero no eixo \textit{Y}, mantendo o padrão de baixo \text{RTT} encontrado na análise geral. No entanto, o gráfico exibe \textit{outliers} severos no eixo \textit{Y}, com valores negativos de \text{RTT} em torno de $-1.0$ segundos, o que é fisicamente impossível para um tempo de latência. Tais pontos indicam falhas no processo de medição ou na coleta de dados, devendo ser tratados como anomalias ou erros de \text{data cleaning} antes de qualquer inferência. Excluindo essas anomalias, o \texttt{server02} demonstra uma perda de pacotes que varia de $0\%$ a aproximadamente $20\%$, com \text{RTT} consistentemente baixo.
